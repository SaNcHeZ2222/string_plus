...
int value_to_string(Specif *sp, va_list ptr, const char *str, int *countB,
                    int count);
int switch_feg(char *str, Specif *sp);
int logicI_du(Specif *sp, char *str, char *suc, int count, int mark,
              int notion);
int set_notion(Specif *sp);
int mod_din(void *pointer, Specif *sp);
int mod_uxo(void *pointer, Specif *sp);
...
void set_value(va_list ptr, Specif *sp);
int str_to_str(char *str, Specif *sp);

int is_nanInf_s(const char *str);
int get_int_feg(char *str, Specif *sp);
int get_int_idu(char *str, Specif *sp);

long double get_value_mod_doub(Specif sp, va_list ptr);
long long get_value_modInt(Specif sp, va_list ptr);
long long unsigned get_value_mod_uInt(Specif sp, va_list ptr);

char *strchrsc(const char *str, int c);

int get_value_s(char *str, Specif *sp);
int init_struct_s(char *str, Specif *sp);
...
int s21_sscanf(const char *str, const char *format, ...) {
  ...
  while (*format) {
    if (*format == '%') {
      ...
      format += a;
      int step = countB;
      count = value_to_string(&sp, ptr, str, &countB, count);
      str += countB - step;
    } else {
      if ((is_white_space(*format) || *format == *str) && flags) {
        if (*format != *str) {
          format++;
        } else {
          format++;
...
int value_to_string(Specif *sp, va_list ptr, const char *str, int *countB,
  ...
  int size = 0;
  sp->count = count;
  while (is_white_space(*str) && 'c' != sp->spec) {
    str++;
    (*countB)++;
  }
  ...
  *sp = qwer;
  while (*str != '\0' && !err) {
    if (*str == '*') {
      ...
      str += parse_to_int_s(str, &(sp->width));
      sp->setWidth = 1;
    }
